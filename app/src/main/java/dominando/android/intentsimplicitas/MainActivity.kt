...
class MainActivity : AppCompatActivity() {
    ...
    private fun openIntentAtPosition(position: Int) {
        ...
        when (position) {
            ...
            1 -> { // Realiza uma chamada
                ...
                if (ActivityCompat.checkSelfPermission(this, permission.CALL_PHONE)
                    ...
                    ActivityCompat.requestPermissions(
                        this,
                        arrayOf(permission.CALL_PHONE), 0
                    )
                } else {
                    callNumber()
                }
            ...
            2 -> { // Pesquisa uma posição do mapa // Seu emulador/aparelho deve ter o Google Maps
            ...
            4 -> { // Compartilhar
                intent = Intent().setAction(Intent.ACTION_SEND)
                    .putExtra(Intent.EXTRA_TEXT, "Compartilhando via Intent.").setType("text/plain")
                openIntent(intent)
            }
            5 -> { // Alarme
                intent = Intent(AlarmClock.ACTION_SET_ALARM).putExtra(
                    AlarmClock.EXTRA_MESSAGE, "Estudar Android"
                ).putExtra(AlarmClock.EXTRA_HOUR, 19).putExtra(AlarmClock.EXTRA_MINUTES, 0)
                    .putExtra(AlarmClock.EXTRA_SKIP_UI, true).putExtra(
